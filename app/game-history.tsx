              ...
              {(data.leaderboard ?? []).slice(0, 10).map((p: any, i: number) => (
                <View key={p.id} style={styles.leaderRow}>
                  ...
                  <Text style={styles.playerName} numberOfLines={1}>
                    {p.user?.displayName?.trim() || p.user?.name?.trim() || `Player #${p.userId}`}
                  </Text>
              ...
              {(data.killFeed ?? []).slice(0, 15).map((event: any) => (
                <View key={event.id} style={styles.feedRow}>
                  <Text style={styles.feedText}>
                    {event.eventType === "elimination_approved" && event.actorName && event.targetName
                      ? `💀 ${event.actorName} eliminated ${event.targetName}!`
                      : event.message}
                  </Text>
